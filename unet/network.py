...
class UNetLayer(nn.Module):
    ...
    def __init__(self, num_channels_in, num_channels_out, ndims,
        ...
        if not self.two_sublayers:
            self.unet_layer = nn.Sequential(conv1, relu1)
        else:
...
class UNetClassifier(UNet):
    ...
    def linear_output(self, x):
        return super(UNetClassifier, self).forward(x)
    ...
    def loss(self, input, labels, weights=None):
        """Default loss for the U-Net used as a classifier."""
        pred = self(input)
        return log_likelihood_loss(pred, labels, weights)
...
class UNetRegressor(UNet):
    ...
    def __init__(self, unet_config=None):
        super(UNetRegressor, self).__init__(unet_config)

    @numpytorch
    def forward(self, x):
        return super(UNetRegressor, self).forward(x)

    @numpytorch
    def loss(self, input, real, weights=None):
        """Default loss for the U-Net used as a regressor."""
        pred = self(input)
        return mse_loss(pred, real, weights)
    

def mse_loss(pred, real, weights=None):

    if weights is None:
        return ((pred - real)**2).mean()

    return (weights * (pred - real)**2).sum() / weights.sum()


def log_likelihood_loss(pred, labels, weights=None):
...
def cross_entropy_loss(pred, gtprobs, weights=None):

    loss = - (pred * gtprobs).sum(1)
